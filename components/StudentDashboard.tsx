...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings, onStartWeeklyTest, activeTab, onTabChange, setFullScreen, onNavigate, isImpersonating, onNavigateToChapter, isDarkMode, onToggleDarkMode }) => {
  ...
  // TTS Global State
  const [autoTTS, setAutoTTS] = useState(() => localStorage.getItem('nst_auto_tts') !== 'false');

  const toggleAutoTTS = () => {
      const newState = !autoTTS;
      setAutoTTS(newState);
      localStorage.setItem('nst_auto_tts', newState.toString());
      if (!newState && window.speechSynthesis) window.speechSynthesis.cancel();
      showAlert(newState ? "Auto-Read Enabled" : "Auto-Read Disabled", "INFO");
  };
  ...
  // --- REVISION MORNING PROMPT ---
  useEffect(() => {
      const today = new Date().toDateString();
      const lastPrompt = localStorage.getItem(`nst_revision_prompt_${user.id}`);
      ...
      if (lastPrompt !== today) {
          ...
          if (dueItems.length > 0) {
              showAlert(`Good Morning! You have ${dueItems.length} topics for revision today.`, 'INFO', 'Dedicated Revision Time');
              localStorage.setItem(`nst_revision_prompt_${user.id}`, today);
          }
      }
  }, [user.id, user.revisionSchedule]);
  ...
  const renderMainContent = () => {
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={toggleAutoTTS} className={`p-1.5 rounded-full ${autoTTS ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 text-slate-400'}`}>
                                    {autoTTS ? <Volume2 size={14} /> : <Volume2 size={14} className="opacity-50" />}
                                </button>
                                {/* DISCOUNT BADGE */}
      ...
      // 2. REVISION TAB (Replacing AI STUDIO)
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 shadow-lg z-50 pb-safe">
            <div className="flex justify-around items-center h-16">
                ...
                <button onClick={() => onTabChange('AI_STUDIO')} className={`flex flex-col items-center justify-center w-full h-full ${activeTab === 'AI_STUDIO' ? 'text-blue-600' : 'text-slate-400'} relative`}>
                    <BrainCircuit size={24} fill={activeTab === 'AI_STUDIO' ? "currentColor" : "none"} />
                    <span className="text-[10px] font-bold mt-1">Revision</span>
                    {user.revisionSchedule && user.revisionSchedule.some(i => new Date(i.nextRevisionDate).setHours(0,0,0,0) <= new Date().getTime()) && (
                        <span className="absolute top-2 right-6 w-2.5 h-2.5 bg-red-600 rounded-full border border-white animate-pulse"></span>
                    )}
                </button>
